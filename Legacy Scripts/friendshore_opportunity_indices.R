...
raw_data<-"OneDrive - RMI/New Energy Industrial Strategy - Documents/NEIS Center Asia Trips/Research/Raw Data/"
...
#Trade Data - Comtrade--------------------------------------------
#NB: DO NOT RUN UNTIL 2025 TRADE DATA IS AVAILABLE! USE FILE FROM RAW DATA
...
opportunity_all <- trade_indices_all %>%
  ...
  mutate(opportunity_index_country = safe_scurve(opportunity_index_raw)) %>%
  select(reporter_iso, partner_iso, tech, supply_chain,
         ...
         opportunity_raw, opportunity_index_raw, opportunity_index,opportunity_index_country) %>%
...
friendshore_all <- import_idx_all %>%
  ...
  group_by(reporter_iso) %>%
  mutate(friendshore_index_country=safe_scurve(fsi_adj)) %>%
  select(reporter_iso, partner_iso, tech, supply_chain,
         imp_trade_index, econ_opp_raw,es_need, eo_partner, outbound_index,
         penalty_r, penalty_p, fsi_raw, fsi_adj, friendshore_index,friendshore_index_country) %>%
  arrange(desc(friendshore_index_country))
...
friendshore_ten <- 
  ...
  friendshore_all %>%
  filter(reporter_iso=="VNM",
         !partner_iso %in% c("CHN","RUS"),
         !tech %in% c("Oil","Gas","Coal")) %>%
  ...
  left_join(country_flags %>%
    ...
    select(
      code_1=Code,
      "Friendshoring"       = ci,
      "Index"=friendshore_index_country) %>%
    slice_max(Index, n = 12)
write.csv(friendshore_ten,"Downloads/friendshore_vnm.csv")
...
export_ten <- 
  ...
  select(
    code_1=Code,
    "Export"       = ci,
    "Index"=opportunity_index_country) %>%
  slice_max(Index, n = 12)
write.csv(export_ten,"Downloads/export_vnm.csv")


#Overall Partnership
partnership<-friendshore_all %>%
  ungroup() %>%
  filter(reporter_iso=="VNM",
         !partner_iso %in% c("CHN","RUS"),
         !tech %in% c("Oil","Gas","Coal"),
         !is.na(friendshore_index_country)) %>%
  ungroup() %>%
  group_by(partner_iso) %>%
  slice_max(friendshore_index_country,n=5) %>%
  summarize(friendshore=mean(friendshore_index_country,na.rm=T)) %>%
  left_join(opportunity_all %>%
              filter(reporter_iso=="VNM",
                     !partner_iso %in% c("CHN","RUS"),
                     !tech %in% c("Oil","Gas","Coal")) %>%
              left_join(country_info,by=c("partner_iso"="iso3c")) %>%
              ungroup() %>%
              group_by(partner_iso) %>%
              slice_max(opportunity_index_country,n=5) %>%
              summarize(export=mean(opportunity_index_country,na.rm=T)),
            by=c("partner_iso")) %>%
  rowwise() %>%
  mutate(partnership=mean(
    c_across(c(friendshore,export)),na.rm=T)) %>%
  left_join(country_info,by=c("partner_iso"="iso3c")) %>%
  left_join(country_flags %>%
              mutate(Country=recode(
                Country,"United States of America"="United States",
  ...
  write.csv(partnership,"Downloads/partnership_vnm_all.csv") 

  partnership <- partnership %>%
  ungroup() %>%
  slice_max(partnership,n=12) %>%
  select(Code,country,partnership,friendshore,export) 
write.csv(partnership,"Downloads/partnership_vnm.csv")
