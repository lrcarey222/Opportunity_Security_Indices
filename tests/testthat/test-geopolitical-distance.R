...
testthat::test_that("build_vote_matrix preserves missing votes as NA", {
  source("R/geopolitical_distance/ideal_points.R")
  sub <- data.frame(
    country = c("A", "A", "B"),
    rcid = c(1, 2, 1),
  ...
  testthat::expect_true(is.na(mat["B", "2"]))
  testthat::expect_equal(mat["A", "1"], 1)
  testthat::expect_equal(mat["A", "2"], 0)
})
